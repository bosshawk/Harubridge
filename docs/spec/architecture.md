# 基本設計

**機能をまたいで効く構造**だけを書く。個別機能の実装詳細は書かない —— コードが正。

> ステータス: **技術構成に由来する部分は確定**。
> 要求が TBD のため、データの持ち方と機能構成は未確定。

## この文書に書くこと / 書かないこと

| 書く | 書かない（→ コード） |
| --- | --- |
| 全体をどんな塊に分けているか、その境界 | 各クラス・関数の構造 |
| 塊どうしが何をやり取りするか（約束事） | 具体的な呼び出し手順 |
| データをどこに、どういう原則で持つか | テーブル定義・スキーマ |
| 外部（ゲーム通信・OS）との接点の置き方 | 個別 API のパース処理 |
| 機能追加時に必ず従う構造上の制約 | 個別機能の振る舞い（→ [`external/`](external/)） |

判断の目安: **その記述が古くなったとき、実装者が誤った前提を持つか？**
持つなら書く。単に「今こう書いてある」だけならコードに任せる。

**なぜその構造にしたか**は [ADR](../adr/) に書く。この文書は「いまどういう構造か」を現在形で書く。

## 全体構造

```mermaid
flowchart LR
    subgraph WebView["OS の WebView"]
        G["ゲーム画面<br/>(iframe 階層)"]
        H["注入スクリプト<br/>XHR フック"]
        U["UI (React)"]
    end
    subgraph Rust["Rust コア"]
        P["受信・パース"]
        S["状態"]
        D["永続化"]
    end
    G -.観測.-> H
    H -->|IPC| P --> S --> D
    S -->|IPC| U
```

| 構成要素 | 責務 | 他との境界 |
| --- | --- | --- |
| 注入スクリプト（JS） | ゲームのフレーム内で XHR を観測して Rust へ渡し、ゲーム画面を表示するための CSS を当てる | **ページ内でのみ動く。判断を持たない。観測と表示調整はファイルを分ける** |
| Rust コア | 受信・パース・状態保持・永続化 | ゲームのページを直接触らない |
| UI（React / TypeScript） | 表示と操作 | Rust コアから受け取った状態のみを描画する |

Rust と WebView の橋渡しは Tauri が担う。

UI は複数の OS ウィンドウ（メインウィンドウと機能グループ単位の情報ウィンドウ。
[external/game-screen.md](external/game-screen.md)）に分かれる。
どのウィンドウも Rust コアから受け取った状態だけを描画し、
ウィンドウどうしで状態を直接やり取りしない。

## 外部との接点

ゲームとの接点は注入スクリプト 1 本に集約する。役割は 2 つあり、
**互いに混ぜない**（ファイルを分ける）。

| 役割 | 中身 | 方向 |
| --- | --- | --- |
| 観測 | XHR を購読し、`/kcsapi/` の応答を Rust へ渡す | ページ → アプリ |
| 表示調整 | ゲーム画面を見せるための CSS をページに当てる | アプリ → ページ |

### 観測

ゲームの通信を知る唯一の手段は**注入スクリプトによる XHR の観測**である。

- **全フレームに注入する。** ゲーム本体は DMM のページ内の iframe
  （`osapi.dmm.com` → `*.kancolle-server.com`）で動作しており、
  メインフレームだけに注入しても `/kcsapi/` を観測できない
- 観測対象は `/kcsapi/` 配下のみ。レスポンスは `svdata=` を除去して JSON として扱う
- **絞り込みは注入スクリプトの中で行う。`/kcsapi/` 以外は Rust コアへ渡さない。**
  フックはゲーム以外の通信（広告配信・トラッキング・SNS ウィジェット等）も観測できてしまう。
  実測では `/kcsapi/` 18 件に対し、それ以外が 216 件だった（2026-08-02）。
  IPC を通してから捨てるのでは、無関係な通信内容が一度アプリ側に渡る。
  **境界はページ内に置く**（[C-04](constraints.md)）
- **`fetch` と WebSocket は使われていない**（実測。2026-08-02）。
  ただし将来使われる可能性はあるため、`TODO(要検証)` として定期的に確認する

### 表示調整

配信元のページ（DMM）は**別オリジンの `iframe` に埋め込めず**、
かつゲーム本体のフレームだけを切り出すと**課金の導線が成立しない**。
そのため入口ページを丸ごと読み込んだうえで、
**CSS を当ててゲーム画面だけを見せる**（[external/game-screen.md](external/game-screen.md)）。
根拠となる観測は [kancolle/api/overview.md](../kancolle/api/overview.md) にある。

### ページへの介入をどこまで許すか

ページへの介入は、**何を変えるかで 4 段階に分ける。許すのは 1 段目だけ**である。

| 段階 | 何を変えるか | 例 | 扱い |
| --- | --- | --- | --- |
| 1 | **見え方だけ**（手元の描画で完結する） | 表示のための CSS 注入 | **許す** |
| 2 | ページの**挙動** | ページの関数の置き換え、キー入力の抑止 | 個別に判断する（[C-01](constraints.md)） |
| 3 | ページが**受け取る内容** | ゲーム資材の差し替え | 個別に判断する（[C-05](constraints.md)） |
| 4 | ゲームサーバへ**送る内容** | Cookie の書き込み、リクエストの改変・再送 | 個別に判断する（[C-02](constraints.md)） |

**2 段目以降は禁止ではない。** 用途が生じた時点で、
その用途ごとに判断を下し、決定を `docs/adr/` に記録してからでなければ実装しない。
**判断を経ずに 2 段目以降へ踏み込む変更は、この構造への違反として扱う。**

ページから値を**読むだけ**の操作（スクリーンショットのための `canvas` の読み取りなど）は
ページを変えないため、この段階分けの対象外である。

### 読み取り専用を構造として保証する方法

[C-02](constraints.md)（通信は読み取り専用）は、次の構造で担保する。

- 注入スクリプトは `XMLHttpRequest` を**サブクラス化し、`loadend` を購読するだけ**とする。
  リクエストの停止・改変・再送・追加送信を行わない
- **プロキシを置かない。** ゲームとサーバの通信経路に一切介入しない
- 送信系の API（`open` / `send`）は引数を記録するのみで、内容を書き換えない

この方針を破る変更は、[C-02](constraints.md) 違反として扱う。

### 非公開仕様への依存を局所化する境界

- 艦これの API 構造に依存する知識は**パース層に閉じ込める**（[C-03](constraints.md) / NFR）
- 未知のフィールド・構造変化を受け取っても**停止せず、既知の範囲で処理を続ける**。
  縮退はパース層で行い、それより内側へ不完全なデータを流さない

## データの持ち方

`TODO(未確定)`: 要求（[requirements.md](requirements.md)）が TBD のため保留。

決まった時点で書くこと:

- 何を永続化し、何をメモリに留めるか
- 保存場所（ユーザーから辿れること）
- 認証情報を保存しない仕組み（[constraints.md](constraints.md) C-04 / C-07）

**確定している制約**: `api_start2/getData` は 2MB 超（実測）。
マスタデータは毎回取得されるため、扱い方を設計時に決める必要がある。

## 外部データの取り込み

艦これのゲームデータのうち、**観測して積み上げた知識で、アプリが実行時に使うもの**は
[`data/`](../../data/) に置く（任務テーブルなど）。

| 置き場所 | 中身 | 誰が読むか |
| --- | --- | --- |
| `data/` | 機械可読なデータ（**正**） | アプリ |
| `docs/kancolle/` | 意味・出典・読み方 | 人 |

**`data/` の中身はビルド時に検証して埋め込む。**

- `build.rs` で読み、**パースや整合性の検査に失敗したらビルドを落とす**
- 手で書くデータであり必ず誤りが混入するため、**実行時ではなくビルド時に検出する**
- 実行時の差し替えは行わない。**外部から取得する経路も作らない**
  （[C-04](constraints.md)）

代償として、**データを更新するには再ビルドが要る**。
ゲーム更新時にはどのみち他の追随も発生するため、これは許容する。

## 機能追加時の構造上の制約

- **ゲームのページへの介入は 1 段目（見え方だけ）に留める。**
  2 段目以降へ踏み込むときは、先に判断を下して `docs/adr/` に記録する
- **UI はゲームの通信を直接見ない。** 必ず Rust コアを経由する
- 新しい観測対象を増やす場合も、経路は注入スクリプト 1 本に保つ
- **`data/` に値を置き、`docs/` には説明だけを置く。** 値を 2 箇所に書かない

## 未解決事項

- `TODO(未確定)`: データの持ち方（上記）
- ~~Rust と TypeScript の間の型の受け渡し方法~~ → `tauri-specta` に決定
- `TODO(要検証)`: Windows（WebView2）での動作確認。macOS のみ実測済み
