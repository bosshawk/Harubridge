...
**ディレクトリ構成と層の話はここに書かない。** リポジトリの実体が正である。
...
| G-TS-11 | MUST | **`api_*` 由来の語を `src/` に持ち込まない**（glossary 原則 2）。UI に届く時点で艦これの語彙は消えている |
...
| G-TS-24 | MUST | **`setInterval` を個々のコンポーネントに置かない。** 毎秒の tick はアプリ全体で 1 本だけ |
...
| G-TS-27 | MUST | selector の中で新しいオブジェクト・配列を作らない。複数の値をまとめて取るときは `useShallow` を使う |
...
| ID | 強度 | ルール |
| --- | --- | --- |
| G-TS-30 | MUST | **`invoke` / `listen` を `ipc/` の外に書かない**。コンポーネントもフックも直接呼ばない |
...
| G-TS-37 | MUST | **マスタデータを UI に持ち込まない。** 艦名・装備名の解決を UI 側で行わない（名前解決は Rust 側で済ませる） |
...
| G-TS-41 | MUST | パネルの外枠（最終更新時刻の表示・E-01 の案内・エラー境界）は **`shared/` の共通部品を使う。** パネルごとに書き直さない |
...
| G-TS-73 | MUST | 送り口の分岐（`window.webkit.messageHandlers` / `window.chrome.webview`）を**1 箇所にまとめる** |
...
| G-TS-85 | MUST | テストの中で現在時刻を直接読まない。**時刻を引数で渡す**（「絶対時刻 − 現在時刻」の計算をそのまま関数にする） |
...
なぜ悪いのか: 前者は「計算は Rust 側で済ませる」に反し、**計算式の出典（NFR-009）が 2 箇所に分裂する。**
...
`eslint` + `prettier` は、
**型情報つきのリント**を有効にする前提で採用されている。

| ID | 強度 | ルール |
| --- | --- | --- |
| G-TS-90 | MUST | **推奨セットで検出できるものを、このガイドラインに書き写さない。** ルールの重複は必ず片方が腐る |
| G-TS-91 | MUST | `typescript-eslint` の**型情報を要求する推奨セット**（`recommended-type-checked` 以上）を土台にする。この検出力のために `biome` ではなく eslint が選ばれている |
| G-TS-92 | MUST | `eslint-plugin-react-hooks` の推奨セットを入れる（Rules of Hooks の機械検出） |
| G-TS-93 | MUST | **`import/no-restricted-paths` の zone 定義を消さない・緩めない。** これが境界（`shared → features → app` の一方向）を守る唯一の手段である |
...
| G-TS-97 | MUST | `pnpm lint` が通らない状態でコミットしない（`main` に直接コミットするため） |
...
- 上記以外に例外を認めない。**特に G-TS-31（マージを書かない）と G-TS-40（境界はパネル単位）は
  例外なし。** どちらも破った時点で状態同期の設計と外部仕様 E-02 が成立しなくなる

## 未解決事項

- `TODO(未確定)`: **`react-error-boundary` の採否**（G-TS-46）。
  React 公式が案内しているが、採用済みの依存一覧に無い。
  **依存を 1 つ増やす判断であり、承認が要る**
- `TODO(未確定)`: **`eslint-plugin-jsx-a11y` の採否**（G-TS-54）。同上。
  なお `eslint-plugin-import`（`import/no-restricted-paths`）は導入済みである
