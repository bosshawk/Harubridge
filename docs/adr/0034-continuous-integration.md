...
**Windows で「動く」ことを VM で一度確認できたあと。**
そこで初めて、CI は「動くと分かっている状態が壊れていないこと」を守る道具として機能する。

順序を逆にしてはいけない。

1. **VM で動作確認する** —— ここでしか「動くかどうか」は分からない（Issue #6）
2. **その状態を CI が守る** —— 共有コードを変えたときに、
   OS 別分岐のコンパイルが壊れていないかを VM を起動せずに検出する

確認できていない段階で 1 を飛ばして 2 だけ置くと、
**緑になっても何も保証せず、赤くなっても直せないジョブ**が残る。

足すときに ADR は起こさない（ワークフローに数行足すだけであり、覆すコストが低いため）。
...
- 却下理由: **いま得られるものが「12 行の Tauri 雛形がコンパイルできる」ことだけ**であり、
  ...
  VM 実行の部分集合ですらなく、単に無意味である**（上記「OS 別のジョブを足す契機」）。
