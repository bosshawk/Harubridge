...
**CI が担える役割は、「動くと分かっている状態が壊れていないこと」の確認である。**

- したがって Windows のジョブが意味を持つのは、**VM で一度「動く」と確認できたあと**である。
  それ以前は、緑になっても何も保証せず、赤くなっても直せない
- **Windows で確認する体制は現在存在しない**（Issue #6。macOS 環境で VM が要り、
  その画面はエージェントから見えない。オーナー判断で後回し）
- したがって現時点で Windows のジョブを置くと、**環境起因で落ちた場合に
  再現もデバッグもできず、赤が `main` に残り続ける**

**いま OS 別のジョブを置いて得られるものは、実際には何か。**
...
### Windows の動作確認を CI に負わせない

**Windows で確かめたいことは、CI では確かめられない。**
...
その担保は Issue #6 が負う。**本 ADR はそれを代替しないし、代替を試みない。**

### OS 別のジョブが将来必要になるか —— **筋書きを描けていない**

**「いずれ必要になったら足す」という書き方をしない。**
その「いずれ」が具体的に描けないためである。将来の予定として書けば、
根拠のないまま既定路線になる。

VM で動作確認したあとに CI が担保できるのは、
**`#[cfg(target_os = "windows")]` のコードがコンパイルできることだけ**である。
振る舞いは守れない。そしてそのコンパイルの回帰は、次に VM を開いたときか、
**Windows 版を配るためのビルドを作るときに分かる。**

- **Windows のビルドは、配布の決定に吸収される見込みが高い。**
  対応 OS に Windows を挙げている以上いずれ配ることになり、
  そのリリースの工程は Windows でビルドする（署名・パッケージングと同じ場所）。
  **配布は本 ADR の対象外**であり、そちらを決めるときに一緒に判断する
- **VM 確認後に繰り返し必要になるのは CI ではなく VM の再確認である。**
  コードが変われば振る舞いも変わりうるが、それは何度確認しても CI では分からない。
  **CI を足しても VM を開く回数は減らない**

したがって本 ADR は、Windows ジョブについて**足す予定も、足さない決定も書かない。**
必要になったときに、そのときの根拠で判断する（ワークフロー数行であり、覆すコストは低い）。
...
- 却下理由: **いま得られるものが「12 行の Tauri 雛形がコンパイルできる」ことだけ**であり、
  ...
  加えて、**そのコードが入ったとしても「動くか」は CI では分からず VM が要る。**
  VM で動かせばコンパイルも当然通るため、**確認体制ができる前の CI は
  VM 実行の部分集合ですらなく、単に無意味である。**
  確認体制ができたあとに何を担保できるかも、現時点では描けていない
  （上記「OS 別のジョブが将来必要になるか」）。
...
**そもそも CI は「動くと分かっている状態が壊れていないこと」を守る道具であり、
「動くかどうか」を確かめる道具ではない。**
Windows については、その「動くと分かっている状態」が一度も存在したことがない。
先に要るのは CI ではなく VM である（Issue #6）。
...
- 配布（署名・公証・自動更新）とリリースの自動化は本 ADR の対象外。
  ...
  **Windows でビルドする工程が要るとすれば、まずここに現れる**
- **Windows の動作確認（Issue #6）を CI は担保しない。**
  必要なのは VM であり、その体制ができたあとに CI が何を守れるかも現時点では描けていない。
  **描けるようになるまで、Windows のジョブについて予定を書かない**
