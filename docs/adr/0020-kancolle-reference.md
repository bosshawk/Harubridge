...
### 3. 記述は `docs/`、データは `data/`

**アプリが実行時に使うデータは `docs/` に置かない。**

| 種類 | 例 | 置き場所 | 形式 |
| --- | --- | --- | --- |
| **仕様の記述**（人が読む） | API の構造、計算式、ルールの説明 | `docs/kancolle/` | Markdown |
| **アプリが読むデータ** | 任務テーブル | **`data/`** | 機械可読（JSON） |

理由は 2 つ。**配布物に `docs/` は入らない**こと、
そして Markdown の表に数百件を書くと**アプリ側に別途データを持つことになり、
二重管理が発生する**こと（[ADR-0008](0008-code-as-source-of-truth.md) が警戒したもの）。

`src-tauri/` にも置かない。**ここにあるのは外部システムの観測結果であり、
ランタイムの選択（[ADR-0016](0016-tech-stack.md)）より長生きする。**

**出典は各レコードの中に持たせる**（`source` / `verified`）。
データが `docs/` から離れても、根拠が一緒に移動するようにするため。

### 4. 形式は Markdown。OpenAPI は採らない
