...
- **`shared/` の定義は「ドメインを知らないコード」に限る。判定は機械で行う**:
  `shared/` は `bindings.ts` / `store/` / `ipc/` を **import できない**
  （`import/no-restricted-paths` の zone に含める）。
  パネルの外枠・エラー境界・書式化ユーティリティはこの定義を満たす。
  艦これの型を知る表示部品（例: 艦名バッジ）は満たさないため `shared/` に置けない
- **ドメインを知る共有部品の扱い**: まず所有する feature に置き、他 feature との重複を許容する
  （2 人目が現れるまで共通化しない。誤った共通化より重複が安い）。
...
- ~~`TODO(承認待ち)`: **`eslint-plugin-import` の導入**~~
  **→ 解消（2026-08-09）。** 本 ADR の採択により承認され、骨格作成時に導入した。
  境界 3 本（features 間 / shared のドメイン遮断 / `@tauri-apps/api` の ipc 外使用）が
  違反時にリントエラーになることを実際の違反コードで確認済み
...
- `TODO(未確定)`: ドメインを知る表示部品の重複が実際に密集した場合の置き場所
  （FSD の `entities/` 相当を起こすか）。事実が生まれるまで決めない（§3）
