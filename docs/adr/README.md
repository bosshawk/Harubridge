...
- **ADR への参照はこのディレクトリの内側だけ。**
  `docs/adr/` の外（spec / guidelines / README / コード / 設定 / スキル）では、
  個別 ADR へのリンク・「ADR-番号」という言及・
  **「経緯は ADR にある」のような ADR を送り先にする言及**をすべて禁止する。
  ADR は過去の経緯の記録であり最新の資料ではない。外の文書は現行のルールを
  自立した形で書く（根拠を示したければ根拠そのものを書く）。
  外に書いてよいのは「決定は `docs/adr/` に記録する」という手続きの説明だけ。
  点検は `audit-adr-refs` スキル。
  （ADR どうしの相互リンク・この一覧からのリンクは従来どおり）
...
| `Withdrawn` | **決定に至らないまま取り下げた。決定は存在しない** | **`archive/`** |

`Withdrawn` は「書くのが早すぎた」ことを認めるためのステータスである。
`Rejected`（採用しないという**決定**）とは別物で、こちらは何も決まっていない。
取り下げるときは次を守る。

1. **決定・決め手・影響の各節を削除する。** 読んだ人が決定を拾えない状態にする
2. 冒頭に警告を付け、**論点の引き継ぎ先（GitHub Issue）**を書く
3. 調査記録（実測値・一次ソースの読解・選択肢の比較）は**残す**。
   捨てると同じ調査を繰り返すことになる。ただし「採用」「却下」の判定は落とす
4. `archive/` へ移し、上記の移動手順に従ってリンクを張り替える。
   このとき、**取り下げた ADR を根拠にしていた記述が他に無いか**を確認する。
   あれば、その記述が自立するように現行の根拠を書き直す（`Withdrawn` を根拠にしない）
...
| [0024](archive/0024-state-sync-granularity.md) | Rust コアから UI への状態同期の粒度 | **Withdrawn**（決定に至らず取り下げ） | 2026-08-03 |
...
| [0036](0036-game-page-intervention-scope.md) | ゲームページへの介入を段階に分け、許すのは「見え方だけ」に留める | Accepted | 2026-08-16 |
