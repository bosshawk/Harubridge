...
**強制される境界は次の 2 本だけとし、それ以外は規約ではなく「置き場所の目安」とする。**
...
| module | 責務 |
| --- | --- |
| 受信・デシリアライズ | 艦これ JSON の外部モデル。`serde` の型がそのまま置かれる |
| 内部モデル | アプリが表示・計算に使う形。**我々が所有する唯一の形** |
| （外部 → 内部の変換関数） | **腐敗防止層の実体。層ではなく関数の集合として置く** |
...
- **イベント 1 本 = ストア 1 つ。** 中身は「全量・観測時刻・未観測フラグ」のみとし、
